...
def main():
    ...
    for seq_name in seqs:
        ...
        im_ = cv2.imread(os.path.join(cfg.DATA_ROOT, 'JPEGImages/480p/',seq_name,'00000.jpg'))
    ...
    with torch.no_grad():
        with DavisInteractiveSession(host=host,
            ...
            while sess.next():
                ...
                if len(annotated_frames(scribbles))==0: #if no scribbles return, keep masks in previous round
                    ...
                    if first_scribble: #if in the first round, extract pixel embbedings.
                        if sequence not in seen_seq:
                            ...
                            embedding_memory=[]
                            test_dataset = DAVIS2017_Feature_Extract(root=cfg.DATA_ROOT, transform=composed_transforms,seq_name=sequence)
                    ...
                    if is_save_image:
                        ...
                        if not os.path.exists(os.path.join(cfg.RESULT_ROOT,sequence,'interactive'+str(n_interaction),'turn'+str(inter_turn))):

                            os.makedirs(os.path.join(cfg.RESULT_ROOT,sequence,'interactive'+str(n_interaction),'turn'+str(inter_turn)))
                        im_.save(os.path.join(cfg.RESULT_ROOT,sequence,'interactive'+str(n_interaction),'turn'+str(inter_turn),'inter_'+ref_img_name+'.png'))
                    ...
                    if first_scribble:
                        ...
                        prev_label= None
                        prev_label_storage=torch.zeros(104,h,w)
                    ...
                    if not first_scribble and torch.unique(scribble_label).size(0)==1:
                        ...
                        if is_save_image: #save image
                            ...
                            if not os.path.exists(os.path.join(cfg.RESULT_ROOT,sequence,'interactive'+str(n_interaction),'turn'+str(inter_turn))):
                                os.makedirs(os.path.join(cfg.RESULT_ROOT,sequence,'interactive'+str(n_interaction),'turn'+str(inter_turn)))
                            im.save(os.path.join(cfg.RESULT_ROOT,sequence,'interactive'+str(n_interaction),'turn'+str(inter_turn),imgname+'.png'))
                        #######################################
                        if first_scribble:
                        ...
                        for ii in range(start_annotated_frame+1,total_frame_num):
                            ...
                            tmp_dic, eval_global_map_tmp_dic,local_map_dics= model.prop_seghead(ref_frame_embedding,prev_embedding,
