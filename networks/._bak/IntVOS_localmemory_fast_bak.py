...
#        x = F.interpolate(x, size=(int(ori_height/2), int(ori_width/2)), mode='bilinear', align_corners=True)


        y = y.permute(2, 0, 1).unsqueeze(0)
#        y = F.interpolate(y, size=(int(ori_height/2), int(ori_width/2)), mode='bilinear', align_corners=True)
        ...
        for y_start in range(2 * max_distance + 1):
            ...
            for x_start in range(2 * max_distance + 1):
                ...
                dist = torch.sum(torch.pow((x - offset_y), 2), dim=1)
                dists.append(dist)
...
#        dists = F.interpolate(dists, size=(ori_height, ori_width), mode='bilinear', align_corners=True)
    ...
    ###############
        ...
        for n in range(bs):
            ...
            nn_features_n, ref_obj_ids = nearest_neighbor_features_per_object(reference_embeddings=seq_ref_frame_embedding, query_embeddings=seq_current_frame_embedding, 
                reference_labels=seq_ref_scribble_label, k_nearest_neighbors=k_nearest_neighbors, gt_ids=gt_ids[n], n_chunks=1)
