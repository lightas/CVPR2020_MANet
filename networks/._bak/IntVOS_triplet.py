...
def _triplet_loss(ref_frame_embedding=None,current_frame_embedding=None,
                ref_scribble_label=None,current_frame_label=None,select_num=100,margin=cfg.TRAIN_TRIPLET_MARGIN):
    """
    reference_embeddings: Tensor of shape [height, width, embedding_dim],
    the embedding vectors for the reference frame.
    ...
    the reference frame.
    """
    ref_objs=torch.unique(ref_scribble_label)
    cur_objs=torch.unique(current_frame_label)

    embedding_dim=ref_frame_embedding.size()[-1]
    ref_frame_embedding_flat=ref_frame_embedding.view(-1,embedding_dim)
    current_frame_embedding_flat = current_frame_embedding.view(-1,embedding_dim)
    ref_scribble_label_flat=ref_scribble_label.view(-1)
    current_frame_label_flat=current_frame_label.view(-1)
    triplet_loss=0.
    if len(ref_objs)==1 or len(cur_objs)==1:
        triplet_loss=0.
        return triplet_loss
    else:
#        print(ref_objs)
#        print(cur_objs)
        count=0
        for obj in ref_objs:
#            if obj!=0 and obj in cur_objs:
            if  obj in cur_objs:
                index_list = torch.arange(len(ref_scribble_label_flat))
                index_list=index_list.cuda()
                index_ = torch.masked_select(index_list,ref_scribble_label_flat==obj)        
                index_=index_.long()
                ref_scribble_label_flat_=torch.index_select(ref_scribble_label_flat,0,index_)
                ref_frame_embedding_flat_=torch.index_select(ref_frame_embedding_flat,0,index_)
                #print(ref_scribble_label_flat_.size(0))
                if ref_scribble_label_flat_.size(0)>=select_num:
                    index_=np.random.choice(ref_scribble_label_flat_.size(0),select_num,replace=False)
                    index_=torch.from_numpy(index_).long()
                    ref_frame_embedding_flat_select = ref_frame_embedding_flat_[index_]
                else:
                    ref_frame_embedding_flat_select=torch.zeros(select_num,embedding_dim)
                    ref_frame_embedding_flat_select=ref_frame_embedding_flat_select.cuda()
                    ref_frame_embedding_flat_select[:ref_frame_embedding_flat_.size(0)]=ref_frame_embedding_flat_
                    ref_frame_embedding_flat_select[ref_frame_embedding_flat_.size(0):]=ref_frame_embedding_flat_[0]
#                print(ref_frame_embedding_flat_select.size())
                index_list = torch.arange(len(current_frame_label_flat))
                index_list=index_list.cuda()
                index_ = torch.masked_select(index_list,current_frame_label_flat==obj)        
                index_=index_.long()
               
                current_frame_embedding_select=torch.index_select(current_frame_embedding_flat,0,index_)
                
                ####
                negative_index_ = torch.masked_select(index_list,current_frame_label_flat!=obj)
                negative_index_ = negative_index_.long()
                negative_current_frame_embedding_select=torch.index_select(current_frame_embedding_flat,0,negative_index_)
                 
                ###
#                print(current_frame_embedding_select.size())

#                print('cur_emb type:{}'.format(current_frame_embedding_select.type()))
#                print('ref_emb type:{}'.format(ref_frame_embedding_flat_select.type()))
                dists=_flattened_pairwise_distances(current_frame_embedding_select,ref_frame_embedding_flat_select)
                ####
                n_dists = _flattened_pairwise_distances(negative_current_frame_embedding_select,ref_frame_embedding_flat_select)
                ####
                #t_loss,_ = torch.min(dists,1)
                #t_loss = torch.mean(t_loss)
                ap,_ = torch.min(dists,1)
                an,_ = torch.min(n_dists,1)
                
                t_loss=F.relu(ap-an+margin)
                t_loss = torch.mean(t_loss)
                triplet_loss+=t_loss
                count+=1
            else:
        ...
        triplet_loss=triplet_loss/(count+1e-5)
        return triplet_loss
...
#        self.semantic_embedding=nn.Sequential(*[self.seperate_conv,self.relu1,self.embedding_conv,self.relu2])
        ...
        if freeze_bn:
            self.freeze_bn()
    def forward(self,x=None,ref_scribble_label=None,previous_frame_mask=None,current_frame_label=None,normalize_nearest_neighbor_distances=True,seq_names=None,gt_ids=None,k_nearest_neighbors=1):
        ...
        tri_loss=self.triplet(ref_frame_embedding,current_frame_embedding,ref_scribble_label,current_frame_label)
        return dic,tri_loss
    ...
    def triplet(self,ref_frame_embedding=None,current_frame_embedding=None,
                ref_scribble_label=None,current_frame_label=None,select_num=cfg.TRAIN_TRI_SELECT_NUM,margin=cfg.TRAIN_TRIPLET_MARGIN):
        ...
        scale_current_frame_label=torch.nn.functional.interpolate(current_frame_label.float(),size=(h,w),mode='nearest')
        scale_current_frame_label=scale_current_frame_label.int()
        final_tri_loss=0.
        for n in range(bs):
            ...
            seq_cur_frame_label = scale_current_frame_label[n].permute(1,2,0)
            t_loss=_triplet_loss(seq_ref_frame_embedding,seq_current_frame_embedding,seq_ref_scribble_label,
                seq_cur_frame_label,select_num,margin)
            final_tri_loss+=t_loss
        final_tri_loss=final_tri_loss/n
        return final_tri_loss
