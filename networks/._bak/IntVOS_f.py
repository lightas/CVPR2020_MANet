...
#                    nn_features_n = (torch.sigmoid(nn_features_n) - 0.5) * 2
        ...
        #            nn_features_n[:,:,:,iii,:]=0.5
            ...
            if cfg.INTER_USE_LOCAL:
                if not first_inter:
                    seq_prev_round_label=scale_prev_round_label[n].permute(1,2,0)
                    prev_round_nn_features_n = local_previous_frame_nearest_neighbor_features_per_object(prev_frame_embedding=seq_ref_frame_embedding, 
                            query_embedding=seq_ref_frame_embedding, prev_frame_labels=seq_prev_round_label,
                                gt_ids=gt_id, max_distance=cfg.MODEL_MAX_LOCAL_DISTANCE)
                else:
