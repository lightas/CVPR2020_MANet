...
#        self.layer3=_split_separable_conv2d(embed_dim,embed_dim)
        ...
        if use_edge:
            self.layer3=_split_separable_conv2d(embed_dim+32,embed_dim)
        else:
...
#        self.semantic_embedding=nn.Sequential(*[self.seperate_conv,self.relu1,self.embedding_conv,self.relu2])
        ...
        if cfg.MODEL_USE_EDGE:
            ...
            self.inter_seghead = IntSegHead(in_dim=cfg.MODEL_SEMANTIC_EMBEDDING_DIM+4)
...
#                to_cat_edge = 0-to_cat_edge
                to_scribble=torch.where(to_cat_scribble_mask_to_cat.float()==1.,to_cat_scribble_mask_to_cat.float(),to_cat_prev_round_to_cat.float())
                filter_= torch.zeros_like(to_cat_prev_round_to_cat)
                to_fill = torch.zeros_like(to_cat_prev_round_to_cat)
                dist=cfg.USE1DIST
                ...
                to_cat_edge_to_cat = torch.where(filter_.byte(),to_cat_edge,to_fill)
...
#            print(to_cat_scribble_mask_to_cat.min())
            ...
            #print(to_cat_edge.size())
                to_cat = torch.cat((to_cat_current_frame_embedding,to_cat_nn_feature_n,to_cat_scribble_mask_to_cat,to_cat_prev_round_to_cat,to_cat_edge_to_cat),1)
