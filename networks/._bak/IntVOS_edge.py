...
#        self.semantic_embedding=nn.Sequential(*[self.seperate_conv,self.relu1,self.embedding_conv,self.relu2])
    ...
    def forward(self,x=None,ref_scribble_label=None,previous_frame_mask=None,normalize_nearest_neighbor_distances=True,seq_names=None,gt_ids=None,k_nearest_neighbors=1):
        ...
        edge,edge_fea = self.edge_layer(x1.detach(),x2.detach(),x3.detach())
        ...
        dic = self.before_seghead_process(ref_frame_embedding,previous_frame_embedding,
                    current_frame_embedding,ref_scribble_label,previous_frame_mask,input_edge,
...
#    def pred_label(self,x):
    def before_seghead_process(self,ref_frame_embedding=None,previous_frame_embedding=None, current_frame_embedding=None,
                ref_scribble_label=None,previous_frame_mask=None,input_edge=None,normalize_nearest_neighbor_distances=True,
...
###                                                          dilate=False)
           ...
           # #########################
            ...
            if cfg.MODEL_USE_EDGE:
                ...
                to_cat_edge = input_edge[n].unsqueeze(0).repeat((ref_obj_ids.size(0),1,1,1))
                to_cat_edge = 0-to_cat_edge
                to_scribble = to_cat_previous_frame
                to_cat_previous_frame = torch.where(to_cat_previous_frame.float()==1.,to_cat_previous_frame.float(),to_cat_edge.float())
                filter_= torch.zeros_like(to_cat_previous_frame)
                to_fill = torch.zeros_like(to_cat_previous_frame)
                dist=cfg.USE1DIST
                for i in range(ref_obj_ids.size(0)):
                ...
                to_cat_previous_frame = torch.where(filter_.byte(),to_cat_previous_frame,to_fill)
