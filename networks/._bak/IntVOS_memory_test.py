...
#        self.semantic_embedding=nn.Sequential(*[self.seperate_conv,self.relu1,self.embedding_conv,self.relu2])
        ...
        if cfg.INTER_USE_GLOBAL:
            self.inter_seghead = DynamicSegHead(in_dim=cfg.MODEL_SEMANTIC_EMBEDDING_DIM+4)
        else:
...
#            print('local time:{}'.format(t5-t4))
            ...
            to_cat_prev_frame_nn_features_n = prev_frame_nn_features_n.squeeze(0).permute(2,3,0,1)
    
            to_cat_current_frame_embedding = current_frame_embedding[n].unsqueeze(0).repeat((ref_obj_ids.size(0),1,1,1))
            memory_key_prev_f= torch.cat((to_cat_current_frame_embedding,to_cat_prev_frame_nn_features_n),1)
            memory_key_prev_f= self.memory_keyhead(memory_key_prev_f)
            ...
            if local_map_dics is not None:
                local_map_tmp_dic,local_map_dist_dic,local_memory_key_dic = local_map_dics
                ...
                if seq_names[n] not in local_map_tmp_dic:
                    local_map_tmp_dic[seq_names[n]] = torch.zeros_like(to_cat_prev_frame_nn_features_n).unsqueeze(0).unsqueeze(0).repeat(104,9,1,1,1,1)
                    if torch.cuda.is_available():
                        local_map_tmp_dic[seq_names[n]] = local_map_tmp_dic[seq_names[n]].cuda()
                if seq_names[n] not in local_memory_key_dic:
                    local_memory_key_dic[seq_names[n]]=torch.zeros_like(memory_key_prev_f).unsqueeze(0).unsqueeze(0).repeat(104,9,1,1,1,1)
                    if torch.cuda.is_available():
                        local_memory_key_dic[seq_names[n]] = local_memory_key_dic[seq_names[n]].cuda()                    
                local_map_dist_dic[seq_names[n]][frame_num[n]][interaction_num-1]=1.0/(abs(frame_num[n]-start_annotated_frame))
                local_map_tmp_dic[seq_names[n]][frame_num[n]][interaction_num-1]=to_cat_prev_frame_nn_features_n.detach()
                local_memory_key_dic[seq_names[n]][frame_num[n]][interaction_num-1]=memory_key_prev_f.detach()
            ...
            #    prev_frame_nn_features_n = torch.mean(prev_frame_nn_features_n,dim=0,keepdim=True)
                #print(memory_key_prev_f)
                if interaction_num==1:
                    to_cat_prev_frame_nn_features_n = local_map_tmp_dic[seq_names[n]][frame_num[n]][interaction_num-1]
#                    prev_frame_nn_features_n = prev_frame_nn_features_n.unsqueeze(0)
                elif interaction_num==2:
                    to_cat_prev_frame_nn_features_n = local_map_tmp_dic[seq_names[n]][frame_num[n]][:interaction_num]
                    memory_key_prev_f = local_memory_key_dic[seq_names[n]][frame_num[n]][:interaction_num]
                    memory_key_prev_f = torch.nn.functional.softmax(memory_key_prev_f,0)
                    to_cat_prev_frame_nn_features_n = to_cat_prev_frame_nn_features_n*memory_key_prev_f
                    to_cat_prev_frame_nn_features_n = to_cat_prev_frame_nn_features_n.sum(0,keepdim=False)
...
#                        prev_frame_nn_features_n = prev_frame_nn_features_n.unsqueeze(0)
                    _,idx = torch.topk(local_map_dist_dic[seq_names[n]][frame_num[n]][:interaction_num],3)
                    to_cat_prev_frame_nn_features_n = local_map_tmp_dic[seq_names[n]][frame_num[n]][idx]
                    memory_key_prev_f = local_memory_key_dic[seq_names[n]][frame_num[n]][idx]
                    memory_key_prev_f = torch.nn.functional.softmax(memory_key_prev_f,0)
                    to_cat_prev_frame_nn_features_n = to_cat_prev_frame_nn_features_n*memory_key_prev_f
                    to_cat_prev_frame_nn_features_n = to_cat_prev_frame_nn_features_n.sum(0,keepdim=False)
                ...
                #####
                #######
                local_map_dics=(local_map_tmp_dic,local_map_dist_dic,local_memory_key_dic)
...
#            to_cat_prev_frame_nn_feature_n = prev_frame_nn_features_n.squeeze(0).permute(2,3,0,1)
            to_cat = torch.cat((to_cat_current_frame_embedding,to_cat_nn_feature_n,to_cat_prev_frame_nn_features_n,to_cat_previous_frame),1)
...
#                nn_features_n = (torch.sigmoid(nn_features_n) - 0.5) * 2
            ...
            if seq_names[n] not in local_map_dist_dic:
                ...
                if torch.cuda.is_available():
                    local_map_dist_dic[seq_names[n]]=local_map_dist_dic[seq_names[n]].cuda()
            if seq_names[n] not in local_map_tmp_dic:
                local_map_tmp_dic[seq_names[n]] = torch.ones_like(nn_features_n.squeeze(0).permute(2,3,0,1)).unsqueeze(0).unsqueeze(0).repeat(104,9,1,1,1,1)
                if torch.cuda.is_available():
                    local_map_tmp_dic[seq_names[n]] = local_map_tmp_dic[seq_names[n]].cuda()
            if seq_names[n] not in local_memory_key_dic:
                local_memory_key_dic[seq_names[n]]=torch.zeros_like(nn_features_n.squeeze(0).permute(2,3,0,1)).unsqueeze(0).unsqueeze(0).repeat(104,9,1,1,1,1)
                if torch.cuda.is_available():
                    local_memory_key_dic[seq_names[n]] = local_memory_key_dic[seq_names[n]].cuda()
            local_map_dist_dic[seq_names[n]][frame_num[n]][interaction_num-1]=0
            local_memory_key_dic[seq_names[n]][frame_num[n]][interaction_num-1]=-1e5
        ...
        #            nn_features_n[:,:,:,iii,:]=0.5
            ...
            if cfg.INTER_USE_GLOBAL:
                if not first_inter:
                    seq_prev_round_label=scale_prev_round_label[n].permute(1,2,0)
                    prev_round_nn_features_n = local_previous_frame_nearest_neighbor_features_per_object(prev_frame_embedding=seq_ref_frame_embedding,query_embedding=seq_ref_frame_embedding, prev_frame_labels=seq_prev_round_label,gt_ids=gt_id, max_distance=cfg.MODEL_MAX_LOCAL_DISTANCE)
                else:
