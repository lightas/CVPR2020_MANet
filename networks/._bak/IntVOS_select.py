...
##############################################################
def select_prev_label(prev_frame_embedding,prev_frame_label,select_percent=cfg.MODEL_SELECT_PERCENT):
    prev_frame_embedding=prev_frame_embedding.permute(2,0,1).unsqueeze(0)
    prev_frame_label = prev_frame_label.permute(2,0,1).unsqueeze(0)
    _,c,emb_h,emb_w= prev_frame_embedding.size()
    flat_p_f_embedding = prev_frame_embedding.squeeze(0).permute(1,2,0).view(emb_h*emb_w,c)
    prev_frame_embedding= F.avg_pool2d(prev_frame_embedding,(2,2),(2,2))
    _,c,h,w= prev_frame_embedding.size()
    prev_obj_ids = torch.unique(prev_frame_label).float()
    ####
    flat_prev_frame_embedding = prev_frame_embedding.squeeze(0).permute(1,2,0).view(h*w,c)
    #flat_prev_frame_embedding=F.normalize(flat_prev_frame_embedding,p=2,dim=1)
    #self_dist =torch.matmul(flat_prev_frame_embedding,torch.t(flat_prev_frame_embedding))
    self_dist = _flattened_pairwise_distances(flat_prev_frame_embedding,flat_prev_frame_embedding)
    self_dist = F.interpolate(self_dist.unsqueeze(0).view(1,h*w,h,w),size=(emb_h,emb_w),mode='bilinear',align_corners=True)
    self_dist = F.interpolate(self_dist.permute(0,2,3,1).view(1,emb_h*emb_w,h,w),size=(emb_h,emb_w),mode='bilinear',align_corners=True)

    self_dist = self_dist.squeeze().view(emb_h*emb_w,emb_h*emb_w)
    flat_prev_frame_label=prev_frame_label.squeeze(0).permute(1,2,0).view(emb_h*emb_w)

    index_list = torch.arange(len(flat_prev_frame_label))
    index_list=index_list.cuda()
    atten_head=[]
    for oid in prev_obj_ids:
        index_ = torch.masked_select(index_list,flat_prev_frame_label.float()==oid)
        index_=index_.long()
        obj_dist = torch.index_select(self_dist,0,index_)
   #     obj_dist = self_dist[index_]
        obj_dist = torch.index_select(obj_dist,1,index_)


        len_dist= obj_dist.size(0)
        obj_dist= obj_dist.sum(1)
        _, select_index=torch.sort(obj_dist)
#        if int(len_dist*(1-select_percent))>0:
        select_index=select_index[int(len_dist*select_percent):]

        s_index = index_[select_index.long()]
        flat_prev_frame_label[s_index]=-1
    flat_prev_frame_label=flat_prev_frame_label.view(emb_h,emb_w,1)
    return flat_prev_frame_label
def select_prev_attenhead(prev_frame_embedding,prev_frame_label,obj_ids,select_percent=cfg.MODEL_SELECT_PERCENT):
    obj_ids=obj_ids.float()
    prev_frame_embedding=prev_frame_embedding.permute(2,0,1).unsqueeze(0)
    prev_frame_label = prev_frame_label.permute(2,0,1).unsqueeze(0)
    _,c,emb_h,emb_w= prev_frame_embedding.size()
    flat_p_f_embedding = prev_frame_embedding.squeeze(0).permute(1,2,0).view(emb_h*emb_w,c)
    ...
    #self_dist =torch.matmul(flat_prev_frame_embedding,torch.t(flat_prev_frame_embedding))
    self_dist = _flattened_pairwise_distances(flat_prev_frame_embedding,flat_prev_frame_embedding)
    self_dist = F.interpolate(self_dist.unsqueeze(0).unsqueeze(0),size=(emb_h*emb_w,emb_h*emb_w),mode='bilinear',align_corners=True)
    self_dist = self_dist.squeeze()
    flat_prev_frame_label=prev_frame_label.squeeze(0).permute(1,2,0).view(emb_h*emb_w)
   ...
   #         obj_dist = self_dist[index_] 
            ...
            obj_dist= obj_dist.sum(1)
            _, select_index=torch.sort(obj_dist)    
            if int(len_dist*select_percent)>0:
                select_index=select_index[:int(len_dist*select_percent)]
                  
            s_index = index_[select_index.long()]
            flat_embedding = torch.index_select(flat_p_f_embedding,0,s_index)
            flat_embedding = torch.mean(flat_embedding,dim=0,keepdim=True)
        else:
            flat_embedding = torch.zeros(1,c)
            flat_embedding = flat_embedding.cuda()
        atten_head.append(flat_embedding)
    atten_head = torch.cat(atten_head,0)
    return atten_head
...
#    def select_attention_head(self, ref_embedding, ref_label, select_num=64):
#        ref_head = ref_embedding * ref_label
#        ref_head = torch.sum(ref_head, dim=(2,3)) / (torch.sum(ref_label, dim=(2,3)) + EPSILON)
#        return ref_head
...
###                                                          dilate=False)
            ...
            selected_seq_prev_frame_label = select_prev_label(seq_prev_frame_embedding,seq_previous_frame_label)
            prev_frame_nn_features_n = local_previous_frame_nearest_neighbor_features_per_object(prev_frame_embedding=seq_prev_frame_embedding, 
                            query_embedding=seq_current_frame_embedding, prev_frame_labels=selected_seq_prev_frame_label,
                                gt_ids=ref_obj_ids, max_distance=cfg.MODEL_MAX_LOCAL_DISTANCE)
    
           # #########################
            previous_frame_to_cat = (selected_seq_prev_frame_label.float()== ref_obj_ids.float())
...
#                attention_head = self.calculate_attention_head(previous_frame_embedding[n].unsqueeze(0).expand((ref_obj_ids.size(0),-1,-1,-1)),
#                     to_cat_previous_frame)
#            else:
#                attention_head = select_prev_attenhead(seq_prev_frame_embedding,seq_previous_frame_label,ref_obj_ids)

            ###
            ###
#            attention_head=attention_head.unsqueeze(-1)
#            atten_head = F.normalize(attention_head,p=2,dim=1)
#            att_cur_frame_embedding = to_cat_current_frame_embedding.view(-1,c,h*w)
#            att_cur_frame_embedding=F.normalize(att_cur_frame_embedding,p=2,dim=1)
#            
#            
#            att_cur_frame_embedding=torch.sum(att_cur_frame_embedding*atten_head,dim=1,keepdim=True)
#            to_cat_atten_map = att_cur_frame_embedding.view(-1,1,h,w)
