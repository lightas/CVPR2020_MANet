...
#        self.semantic_embedding=nn.Sequential(*[self.seperate_conv,self.relu1,self.embedding_conv,self.relu2])
        ...
        if freeze_bn:
            self.freeze_bn()
    def forward(self,x=None,ref_scribble_label=None,previous_frame_mask=None,prev_round_label=None,normalize_nearest_neighbor_distances=True,seq_names=None,gt_ids=None,k_nearest_neighbors=1):
        ...
        dic = self.before_seghead_process(ref_frame_embedding,previous_frame_embedding,
                    current_frame_embedding,ref_scribble_label,previous_frame_mask,prev_round_label,
...
#    def pred_label(self,x):
    def before_seghead_process(self,ref_frame_embedding=None,previous_frame_embedding=None, current_frame_embedding=None,
                ref_scribble_label=None,previous_frame_mask=None,prev_round_label=None,normalize_nearest_neighbor_distances=True,
...
###                                                          dilate=False)
            ...
            #####################Preround dist map
            if cfg.MODEL_PREROUND_MAP:
                seq_prev_round_label = scale_prev_round_label[n].permute(1,2,0)
                prev_round_nn_features_n = local_previous_frame_nearest_neighbor_features_per_object(prev_frame_embedding=seq_current_frame_embedding,
                                query_embedding=seq_current_frame_embedding, prev_frame_labels=seq_prev_round_label,
                                    gt_ids=ref_obj_ids, max_distance=3) 
                to_cat_prev_round = prev_round_nn_features_n.squeeze(0).permute(2,3,0,1)
                
            else:
                seq_prev_round_label = scale_prev_round_label[n].permute(1,2,0)
                prev_round_to_cat = (seq_prev_round_label.float()== ref_obj_ids.float())
                to_cat_prev_round = prev_round_to_cat.unsqueeze(-1).permute(2,3,0,1).float()
           ...
           # #########################
            ...
            to_cat = torch.cat((to_cat_current_frame_embedding,to_cat_nn_feature_n,to_cat_prev_frame_nn_feature_n,to_cat_previous_frame,to_cat_prev_round),1)
