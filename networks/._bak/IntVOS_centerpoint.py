...
class IntSegHead(nn.Module):
    def __init__(self,in_dim=(cfg.MODEL_SEMANTIC_EMBEDDING_DIM+3),emb_dim=cfg.MODEL_HEAD_EMBEDDING_DIM,out_dim=256):
        super(IntSegHead,self).__init__()
        self.interfuse = InterFuse(in_dim,out_dim)
        self.conv1 = nn.Conv2d(out_dim,emb_dim,kernel_size=7,stride=1,padding=3)
...
def get_center_points(mask):
    padding_pixel=5
#    no_background = (mask==1)
    no_background = mask.squeeze(0)
    _,h,w = mask.size()
    if no_background.sum()<1:
        ...
        center_point=[int((h_min+h_max)/2),int((w_min+w_max)/2)]
        center_mask=torch.zeros_like(mask.squeeze(0))
        for i in range(0,100):
            center_mask[(center_point[0]-i):(center_point[0]+i),(center_point[1]-i):(center_point[0]+i)]=1
            center_pointmask=center_mask * mask.squeeze(0)
            if not torch.sum(center_pointmask)==0 :
                break


            if i ==20:
                center_pointmask=mask.squeeze(0)
                break
        no_b = center_pointmask.nonzero()
        center_point,_ = torch.min(no_b,0)
        center_point = center_point.cpu().numpy()
        top = random.randint(h_min-2*padding_pixel,h_min-padding_pixel)
        bottom = random.randint(h_max+padding_pixel,h_max+2*padding_pixel)
        left = random.randint(w_min-2*padding_pixel,w_min-padding_pixel)
        right = random.randint(w_max+padding_pixel,w_max+2*padding_pixel)
        top = max(0,top)
        bottom = min(h,bottom)
        left = max(0,left)
        right = min(w,right)
        ...
        return flag,[center_point,left_top,left_bottom,right_top,right_bottom]
...
#        self.semantic_embedding=nn.Sequential(*[self.seperate_conv,self.relu1,self.embedding_conv,self.relu2])
        ...
        if cfg.USE_PRE:
            if cfg.CENTER_CAT:
                self.inter_seghead = IntSegHead(in_dim=3)
            else:
                self.inter_seghead = IntSegHead(in_dim=5)
        else:
            if cfg.CENTER_CAT:
                self.inter_seghead = IntSegHead(in_dim=2)
            else:
                self.inter_seghead = IntSegHead(in_dim=4)
...
#            to_cat_nn_feature_n = nn_features_n.squeeze(0).permute(2,3,0,1)
            ...
            #print(to_cat_current_frame_embedding.size())
            #print(to_cat_scribble_mask_to_cat.size())
            #print(to_cat_prev_round_to_cat.size())
            #print(to_cat_background_feature.size())
            to_cat_gaussian_mask=torch.zeros_like(to_cat_scribble_mask_to_cat) 
            to_cat_negative_gaussian_mask = torch.zeros_like(to_cat_scribble_mask_to_cat)
            for obj in gt_id[1:]:
                mask = torch.where(to_cat_scribble_mask_to_cat[obj]==1,to_cat_scribble_mask_to_cat[obj],to_cat_prev_round_to_cat[obj])     
                _,embh,embw=mask.size()
                flag, points =get_center_points(mask)
                #print(points)
                if flag == 0:
#                    to_cat_cat_mask = torch.ones_like(to_cat_scribble_mask_to_cat[obj])*(-1)
                    to_cat_cat_mask = torch.zeros_like(to_cat_scribble_mask_to_cat[obj])
                else:
                    center = points[0]
                    gaussian_map=make_gaussian(size=(embh,embw),center=center)        
                    to_cat_cat_mask = gaussian_map.cuda().unsqueeze(0)
                    if cfg.CENTER_CAT:
                        to_cat_cat_mask = torch.where(to_cat_scribble_mask_to_cat[obj]==1,to_cat_scribble_mask_to_cat[obj],to_cat_cat_mask.float())
                    
                to_cat_gaussian_mask[obj]=to_cat_cat_mask
                    
                    
                if cfg.USE_EXTREME_POINT:
                    if flag==0:
  #                      to_cat_cat_mask=torch.ones_like(to_cat_scribble_mask_to_cat[obj])*(-1)
                        to_cat_cat_mask = torch.zeros_like(to_cat_scribble_mask_to_cat[obj])
                    else:
                        to_cat_cat_mask=torch.zeros_like(to_cat_scribble_mask_to_cat[obj])
                        for epoint in points[1:]:
                            gaussian_map=make_gaussian((embh, embw),center=epoint)
                            gaussian_map = gaussian_map.cuda().unsqueeze(0).float()
                     
                            to_cat_cat_mask = torch.where(gaussian_map>to_cat_cat_mask,gaussian_map,to_cat_cat_mask) 
                        
                        if cfg.CENTER_CAT:
                            to_cat_cat_mask = torch.where(to_cat_background_feature[obj]==1,to_cat_background_feature[obj],to_cat_cat_mask)
                    to_cat_negative_gaussian_mask[obj]=to_cat_cat_mask    
                if cfg.CENTER_CAT:
                    to_cat_gaussian_mask[0]=to_cat_scribble_mask_to_cat[0]
                    to_cat_negative_gaussian_mask[0]=to_cat_background_feature[0]
 
                if cfg.USE_PRE:          
                
                    if cfg.CENTER_CAT:
                    
                        to_cat = torch.cat((to_cat_prev_round_to_cat,to_cat_gaussian_mask,to_cat_negative_gaussian_mask),1)
                    else:
                        to_cat = torch.cat((to_cat_prev_round_to_cat,to_cat_scribble_mask_to_cat,to_cat_gaussian_mask,to_cat_background_feature,to_cat_negative_gaussian_mask),1)
                else:
                    if cfg.CENTER_CAT:
                        to_cat = torch.cat((to_cat_gaussian_mask,to_cat_negative_gaussian_mask),1)
                    else:
                        to_cat = torch.cat((to_cat_scribble_mask_to_cat,to_cat_gaussian_mask,to_cat_background_feature,to_cat_negative_gaussian_mask),1)
                 ##TEST
           ...
           #     to_cat = torch.cat((to_cat_current_frame_embedding,to_cat_nn_feature_n,to_cat_scribble_mask_to_cat,to_cat_prev_round_to_cat),1)
            pred_ = self.inter_seghead(to_cat_current_frame_embedding,to_cat)
            pred_ = pred_.permute(1,0,2,3)
            dic_tmp[seq_names[n]]=pred_
        if local_map_dics is None:
            return dic_tmp
