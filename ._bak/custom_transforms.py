...
#        self.fix = fix
    ...
    def __call__(self, sample):
        ...
        img1 = cv2.resize(img1, dsize=(new_w,new_h), interpolation=cv2.INTER_CUBIC)
        img2 = cv2.resize(img2, dsize=(new_w,new_h), interpolation=cv2.INTER_CUBIC)
        ref_img = cv2.resize(ref_img, dsize=(new_w,new_h), interpolation=cv2.INTER_CUBIC)
        ...
        ref_scribble_label = cv2.resize(ref_scribble_label, dsize=(new_w,new_h), interpolation=cv2.INTER_NEAREST)
        label1 = cv2.resize(label1, dsize=(new_w,new_h), interpolation=cv2.INTER_NEAREST)
        label2 = cv2.resize(label2, dsize=(new_w,new_h), interpolation=cv2.INTER_NEAREST)
        sample['img1']=img1
        sample['img2']=img2
        sample['ref_img']=ref_img
        sample['ref_scribble_label']=ref_scribble_label
        sample['label1']=label1
    ...
    def __call__(self, sample):
        ...
        for scribble in self.scribbles[frame_num]:
            points_scribble = np.round(
                np.array(scribble['path']) * np.array((im_size[1], im_size[0]))).astype(np.int)
        ...
        if self.nocare_area is None:
            nz = np.where(mask > 0)
            nocare_area = int(.5 * np.sqrt((nz[0].max() - nz[0].min()) * (nz[1].max() - nz[1].min())))
        else:
            nocare_area = 100
        ...
        if frame_num == self.first_frame:
            ...
            scr_gt, scr_nocare = helpers.gt_from_scribble(mask, dilation=self.dilation, nocare_area=nocare_area)
            scr_gt_neg, _ = helpers.gt_from_scribble(mask_neg, dilation=self.dilation, nocare_area=None)
        ...
        # For annotation rounds generated by the robot
            ...
            # Compute dilated foreground, background, and no-care area
            scr_gt_extra, _ = helpers.gt_from_scribble(mask, dilation=self.dilation, nocare_area=None)
            scr_gt_neg, _ = helpers.gt_from_scribble(mask_neg, dilation=self.dilation, nocare_area=None)
