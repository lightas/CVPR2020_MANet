...
from networks.IntVOS_memory import IntVOS
 ...
 #       sample['meta']={'seq_name':seqname,'frame_num':frame_num,'obj_num':obj_num}
            for ii, sample in enumerate(trainloader):
                ...
                if damage_initial_previous_frame_mask:
                    try:
                        label1s_1 = damage_masks(label1s)
                    except:
                        label1s_1 = label1s
                        print('damage_error')
                    try:
                    ...
                    try:
                        ...
                        for i in range(bs):
                            ...
                            label1s_tocat = torch.cat((label1s_tocat,l.float()),0)
                        label1s_3 = label1s_tocat
                    except:
                        label1s_3 = label1s
                    label1s_1=label1s_1.unsqueeze(0)
                    label1s_2=label1s_2.unsqueeze(0)
                    label1s_3=label1s_3.unsqueeze(0)
                    label1s=torch.cat((label1s_1,label1s_2,label1s_3),0)
                ####
