...
from networks_old.backbone import build_backbone
from networks_old.ASPP import ASPP
...
#        feature_aspp = self.upsample_sub(feature_aspp,scale_factor=self.cfg.MODEL_OUTPUT_STRIDE//4,mode='bilinear',align_corners = True)
        feature_shallow = self.shortcut_conv(layers[0])
        _,_,h_,w_=feature_shallow.size()
        feature_aspp = self.upsample_sub(feature_aspp,size=(h_,w_),mode='bilinear',align_corners = True)


        feature_cat = torch.cat([feature_aspp,feature_shallow],1)
        result = self.cat_conv(feature_cat) 
        #result = self.cls_conv(result)
        #result = self.upsample4(result,scale_factor=4,mode='bilinear',align_corners = True)
        return result
