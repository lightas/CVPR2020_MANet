...
from dataloaders.davis_2017_f import DAVIS2017_Train,DAVIS2017_VOS_Train,DAVIS2017_VOS_Test
...
from networks.IntVOS_two_branch import IntVOS
...
if cfg.TRAIN_TBLOG:
    ...
    tblogger = SummaryWriter('./log_model_new_total_new_nofbn_global_local')

class Manager(object):
    def __init__(self, use_gpu=True,time_budget=None,
        save_result_dir='./saved_model_new_two_branch_use_true',pretrained=True,interactive_test=False,freeze_bn=False):
...
#                        shuffle=False,num_workers=cfg.NUM_WORKER,pin_memory=True)
        ...
        epoch_per_round=120
        if model_resume:
            saved_model_=os.path.join(self.save_res_dir,'save_step_55000.pth')
                    ...
                    for ii, sample in enumerate(trainloader):
                        ...
                        if damage_initial_previous_frame_mask:
                            try:
                                ...
                                print('damage_error')    
                        if r==0:
                            ref_scribble_labels_ = self.rough_ROI(ref_scribble_labels)
                        else:
                            ref_scribble_labels_=ref_scribble_labels
#                            ref_scribble_label_ = ref_scribble_label_.squeeze()
#                            im = ref_scribble_label_.numpy()
#                            im=Image.fromarray(im.astype('uint8')).convert('P')
#                            im.putpalette(_palette)
#                            im.save('test.png')
#                            ref_scribble_label_ = ref_scribble_labels.squeeze()
#                            im = ref_scribble_label_.numpy()
#                            im=Image.fromarray(im.astype('uint8')).convert('P')
#                            im.putpalette(_palette)
#                            im.save('testo.png')
...
#
                        ...
                        tmp_dic, global_map_tmp_dic= self.model.before_seghead_process(ref_frame_embedding,previous_frame_embedding,
                                            current_frame_embedding,ref_scribble_labels_,label1s,
                                            ...
                                            global_map_tmp_dic=global_map_tmp_dic,frame_num=frame_nums,dynamic_seghead=self.model.dynamic_seghead)
            

#                        tmp_dic,global_map_tmp_dic = self.model(inputs,ref_scribble_labels,label1s,seq_names=seq_names,gt_ids=obj_nums,k_nearest_neighbors=cfg.KNNS)        
                        ...
                        loss_cf = criterion(tmp_dic,label_dic,step)
                        loss_cf =loss_cf/bs
                        ...
                        if r==0:
                            ...
                            tmp_dic = self.model.int_seghead(ref_frame_embedding=ref_frame_embedding,ref_scribble_label=ref_scribble_labels,
                                    prev_round_label=None,normalize_nearest_neighbor_distances=True,global_map_tmp_dic=global_map_tmp_dic,
                                 seq_names=seq_names,gt_ids=obj_nums,k_nearest_neighbors=cfg.KNNS,
                            ...
                            tmp_dic = self.model.int_seghead(ref_frame_embedding=ref_frame_embedding,ref_scribble_label=ref_scribble_labels,
                                    prev_round_label=prev_round_label,normalize_nearest_neighbor_distances=True,global_map_tmp_dic=global_map_tmp_dic,
                                 seq_names=seq_names,gt_ids=obj_nums,k_nearest_neighbors=cfg.KNNS,
                                 frame_num=ref_frame_nums,first_inter=first_inter)
                        label_and_obj_dic={}
                        label_dic={}
                        ...
                        loss_rf = criterion(tmp_dic,label_dic,step)
                        loss_rf =loss_rf/bs
                        loss = (loss_rf+loss_cf)*0.5
...
#                                            print('damage_error') 
                                ...
                                tmp_dic, global_map_tmp_dic = self.model(inputs,ref_scribble_labels,label1s,seq_names=seq_names,
                                                            gt_ids=obj_nums,k_nearest_neighbors=cfg.KNNS,global_map_tmp_dic={},
                    ...
                    if r!=round_-1:
                        ...
                        for ii, sample in enumerate(trainloader):
                            ...
                            label2s__=label2s__.unsqueeze(0)                        
                            frame_num_dic[seq_names[0]]=frame_nums[0]
...
#############################################
    ...
    def test_VOS(self,use_gpu=True):
        ...
        print('model loading...')
        saved_model_dict = os.path.join(self.save_res_dir,'save_step_100000.pth')
...
#        #### b*1*h*w
#        b,_,h,w = ref_scribble_labels.size()
#        no_background = (ref_scribble_labels!=-1)
#        no_background = no_background.squeeze(1)
#        filter_= torch.zeros_like(ref_scribble_labels)
#        to_fill = torch.zeros_like(ref_scribble_labels)
#        for i in range(b):
#            no_b=no_background[i].nonzero()
#            (h_min,w_min),_ = torch.min(no_b,0)
#            (h_max,w_max),_ = torch.max(no_b,0)
#            filter_[i,0,max(h_min-15,0):min(h_max+15,h-1),max(w_min-15,0):min(w_max+15,w-1)]=1
#        final_scribble_labels = torch.where(filter_.byte(),ref_scribble_labels,to_fill)
#        return final_scribble_labels
