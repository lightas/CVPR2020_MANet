...
from networks.IntVOS_memory_test import IntVOS
...
def main():
    ...
    model.eval()
    inter_image_file=os.path.join(report_save_dir,'inter_image.txt')
    inter_image_file=open(inter_image_file,'w')
    seen_seq=[]
    with torch.no_grad():
    ...
    #            torch.cuda.empty_cache()
                ...
                if len(annotated_frames(scribbles))==0:
                    ...
                    inter_image_file.write(sequence+' '+str(n_interaction)+' '+str(start_annotated_frame)+'\n')
