...
from networks.IntVOS_localrefine import IntVOS
...
#        self.model.freeze_bn()
    ...
    def train(self,damage_initial_previous_frame_mask=True,lossfunc='cross_entropy',model_resume=False):
        ...
        running_loss_refine= AverageMeter()
...
#                        shuffle=False,num_workers=cfg.NUM_WORKER,pin_memory=True)
 ...
 #       sample['meta']={'seq_name':seqname,'frame_num':frame_num,'obj_num':obj_num}
            for ii, sample in enumerate(trainloader):
                ...
                tmp_dic,local_refine_dic = self.model(inputs,ref_scribble_labels,label1s,seq_names=seq_names,gt_ids=obj_nums,k_nearest_neighbors=cfg.KNNS)

                label_and_obj_dic={}
                label_dic={}
                label_for_refine_dic={}
                ...
                for seq_ in tmp_dic.keys():
                    ...
                    tmp_local_refine_logits = local_refine_dic[seq_]
                    _,_,emb_h,emb_w = tmp_local_refine_logits.size()
                    label_for_refine_tmp = nn.functional.interpolate(label_tmp.unsqueeze(0).float(),size=(emb_h,emb_w),mode = 'nearest')
                    label_for_refine_tmp = label_for_refine_tmp.squeeze(0)
                    ...
                    if lossfunc == 'bce':
                        ...
                        label_for_refine_dic[seq_]=label_for_refine_tmp.long()
                      



                loss1 = criterion(tmp_dic,label_dic,step)
                loss_refine = criterion(local_refine_dic,label_for_refine_dic,step)
                loss = (loss1+loss_refine)*0.5
                loss =loss/bs
                optimizer.zero_grad()
                loss.backward()
                ...
                running_loss1.update(loss1.item(),bs)
                running_loss_refine.update(loss_refine.item(),bs)
                if step%50==0:
                    #print(torch.cuda.memory_allocated())
                    #print(torch.cuda.max_memory_cached())
                    torch.cuda.empty_cache()
                    #torch.cuda.reset_max_memory_allocated()
                    print('step:{},now_lr:{} ,loss:{:.4f}({:.4f}), loss1:{:.4f}({:.4f}),loss_refine:{:.4f}({:.4f})'.format(step,now_lr ,running_loss.val,running_loss.avg,running_loss1.val,running_loss1.avg,running_loss_refine.val,running_loss_refine.avg))
