...
from networks.IntVOS_localmemory_fast import IntVOS
        ...
        with DavisInteractiveSession(host=host,
            ...
            while sess.next():
                ...
                if len(annotated_frames(scribbles))==0:
                    ...
                    print('evaluating sequence:{} frame:{}'.format(sequence,start_annotated_frame))
                    scr_f=str(scr_f)
                    while len(scr_f)!=5:
                        scr_f='0'+scr_f
                    ...
                    #######
                    #if w_>1000:
                    ref_img=nn.functional.interpolate(ref_img,size=(resized_h,resized_w),mode = 'bilinear',align_corners=True)
                     ######
                    ...
                    if not first_scribble and torch.unique(scribble_label).size(0)==1:
                        ...
                        for ii in range(start_annotated_frame+1,total_frame_num):
                            ...
                            #####
                            #if w>1000:
                            img=nn.functional.interpolate(img,size=(resized_h,resized_w),mode = 'bilinear',align_corners=True)
                            ####
                            img = img.cuda()
                            if first_scribble and inter_turn==1:
