...
from networks.IntVOS_fast import IntVOS
...
if cfg.TRAIN_TBLOG:
    ...
    tblogger = SummaryWriter('./log_global_local')

class Manager(object):
    def __init__(self, use_gpu=True,time_budget=None,
        save_result_dir='./saved_model_total_re',pretrained=True,interactive_test=False,freeze_bn=False):
...
##                    tmp_dic = self.model(inputs,ref_label,prev_label,seq_names=seqnames,gt_ids=obj_num,k_nearest_neighbors=cfg.KNNS)
                    ...
                    ################
                    t1 = time.time()
                    print('pre_time:{}'.format(t1-time_start))
                    ...
                    t3 = time.time()
                    print('seghead time:{}'.format(t3-t2))
...
#                    cv2.imwrite(os.path.join('./result_2',seq_name,imgname[0].split('.')[0]+'.png'),pred_label)
                    ...
                    print('after time:{}'.format(one_frametime-t3))
