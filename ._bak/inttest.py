...
import timeit
import numpy as np 
from dataloaders.davis_2017 import DAVIS2017_Test,DAVIS2017_Test_Manager
...
def main():
    ...
    save_res_dir = './saved_model_inter_net_new'
    ...
    saved_model_dict = os.path.join(save_res_dir,'save_step_120000.pth')
...
#                eval_dataset=DAVIS2017_Test(split='val',seq_name=sequence,transform=tr.ToTensor())
#                eval_dataset.update_ref_frame_and_label(scribbles)
...
#                                shuffle=False,num_workers=0,pin_memory=True)
                ...
                tmp_dic = model.int_seghead(ref_frame_embedding=ref_frame_embedding,ref_scribble_label=scribble_label,prev_round_label=prev_label,
                ...
                #    current_embedding= model.extract_feature(img)
                    ...
                    prev_label =prev_label.cuda()
                    tmp_dic, eval_global_map_tmp_dic= model.before_seghead_process(ref_frame_embedding,prev_embedding,
                                            current_embedding,scribble_label,prev_label,
                                            normalize_nearest_neighbor_distances=True,use_local_map=True,
                                        seq_names=[sequence],gt_ids=torch.Tensor([obj_nums]),k_nearest_neighbors=cfg.KNNS,
                                        global_map_tmp_dic=eval_global_map_tmp_dic,frame_num=[ii],dynamic_seghead=model.dynamic_seghead)
                    pred_label = tmp_dic[sequence]
                ...
                for ii in range(start_annotated_frame):
                    ...
                    tmp_dic, eval_global_map_tmp_dic= model.before_seghead_process(ref_frame_embedding,prev_embedding,
                                            ...
                                            normalize_nearest_neighbor_distances=True,use_local_map=True,
                                        seq_names=[sequence],gt_ids=torch.Tensor([obj_nums]),k_nearest_neighbors=cfg.KNNS,
                                        global_map_tmp_dic=eval_global_map_tmp_dic,frame_num=[current_frame_num],dynamic_seghead=model.dynamic_seghead)
                    pred_label = tmp_dic[sequence]
                    pred_label = nn.functional.interpolate(pred_label,size=(h,w),mode = 'bilinear',align_corners=True)    
