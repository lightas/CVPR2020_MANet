...
##################################
    def train(self,damage_initial_previous_frame_mask=True,lossfunc='cross_entropy',model_resume=False,eval_total=False,init_prev=False):
        ...
        param_group=[{'params':self.model.inter_seghead.parameters()},{'params':self.model.edge_layer.parameters()}]
        
        optimizer = optim.SGD(param_group,lr=cfg.TRAIN_LR,momentum=cfg.TRAIN_MOMENTUM,weight_decay=cfg.TRAIN_WEIGHT_DECAY)
...
#                        shuffle=True,num_workers=cfg.NUM_WORKER,pin_memory=True)
        ...
        if lossfunc=='bce':
            ...
            #criterion2 = AddedEdge_CrossEntropyLoss(cfg.TRAIN_TOP_K_PERCENT_PIXELS,cfg.TRAIN_HARD_MINING_STEP)
...
#                            ref_scribble_labels=self.rough_ROI(ref_scribble_labels)
                        ...
                        ref_frame_embedding,x1,x2,x3 = self.model.extract_feature(inputs)
                        pred_edge,edge_fea = self.model.edge_layer(x1,x2,x3)
                         
                        if cfg.MODEL_USE_EDGE:
                            input_edge=pred_edge
                        else:
                            input_edge=None
                        ...
                        if r==0:
                            ...
                            tmp_dic = self.model.int_seghead(ref_frame_embedding=ref_frame_embedding,ref_scribble_label=ref_scribble_labels,
                                    prev_round_label=None,input_edge=input_edge,normalize_nearest_neighbor_distances=True,global_map_tmp_dic={},
                        ...
                        #    print(prev_round_label.size())
                            ...
                            tmp_dic = self.model.int_seghead(ref_frame_embedding=ref_frame_embedding,ref_scribble_label=ref_scribble_labels,
                                    prev_round_label=prev_round_label,input_edge=input_edge,normalize_nearest_neighbor_distances=True,global_map_tmp_dic={},
                                 seq_names=seq_names,gt_ids=obj_nums,k_nearest_neighbors=cfg.KNNS,
                                 frame_num=ref_frame_nums,first_inter=first_inter)
...
#                                            print('damage_error') 
                                ...
                                tmp_dic, global_map_tmp_dic,_ = self.model(inputs,ref_scribble_labels,label1s,seq_names=seq_names,
