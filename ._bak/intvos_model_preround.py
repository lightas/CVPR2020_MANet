...
from networks.IntVOS_preround import IntVOS
 ...
 #       sample['meta']={'seq_name':seqname,'frame_num':frame_num,'obj_num':obj_num}
            for ii, sample in enumerate(trainloader):
                ...
                if self.use_gpu:
                    ...
                    label2s_ = label2s_.cuda()
                 
                ##########


                tmp_dic = self.model(inputs,ref_scribble_labels,label1s,label2s_,seq_names=seq_names,gt_ids=obj_nums,k_nearest_neighbors=cfg.KNNS)
...
#                    prev_round_label = torch.ones_like(ref_label)*0.5
...
##                    tmp_dic = self.model(inputs,ref_label,prev_label,seq_names=seqnames,gt_ids=obj_num,k_nearest_neighbors=cfg.KNNS)
                    ...
                    current_frame_embedding,ref_label,prev_label,prev_round_label,
                    True,
