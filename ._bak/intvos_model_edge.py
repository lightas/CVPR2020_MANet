...
from networks.IntVOS_edge import IntVOS
from networks.loss import Added_BCEWithLogitsLoss,Added_CrossEntropyLoss,AddedEdge_CrossEntropyLoss
...
class Manager(object):
    def __init__(self, use_gpu=True,time_budget=None,
        ...
        self.feature_extracter = DeepLab(backbone='resnet_edge',freeze_bn=freeze_bn)
...
#        self.model.freeze_bn()
    ...
    def train(self,damage_initial_previous_frame_mask=True,lossfunc='cross_entropy',model_resume=False):
        ...
        #param_groups=[{'params':self.model.feature_extracter.parameters()},{'params':self.model.semantic_embedding.parameters()},
#                       {'params':self.model.edge_layer.parameters()},{'params':self.model.dynamic_seghead.parameters()}]
        optimizer = optim.SGD(self.model.parameters(),lr=cfg.TRAIN_LR,momentum=cfg.TRAIN_MOMENTUM,weight_decay=cfg.TRAIN_WEIGHT_DECAY,nesterov=True)
...
#                        shuffle=False,num_workers=cfg.NUM_WORKER,pin_memory=True)
        ...
        if lossfunc=='bce':
            ...
            #criterion2 = AddedEdge_CrossEntropyLoss(0.05,cfg.TRAIN_HARD_MINING_STEP)
            criterion2 = AddedEdge_CrossEntropyLoss()
        else:
 ...
 #           sample['meta']={'seq_name':seqname,'frame_num':frame_num,'obj_num':obj_num}
                for ii, sample in enumerate(trainloader):
                    ...
                    #im = Image.fromarray(edges[0].squeeze().numpy().astype('uint8')).convert('P')
                    ...
                    tmp_dic,pred_edge = self.model(inputs,ref_scribble_labels,label1s,seq_names=seq_names,gt_ids=obj_nums,k_nearest_neighbors=cfg.KNNS)
                    #print(tmp_dic)
                    pred_edge=nn.functional.interpolate(pred_edge,size=(h,w),mode = 'bilinear',align_corners=True)
                    label_and_obj_dic={}
                    label_dic={}
                    ...
                    loss_1 = criterion(tmp_dic,label_dic,step)
                    loss_2 = criterion2(pred_edge,edges.float(),step)
                    loss=loss_1+loss_2
                    loss =loss/bs
                    optimizer.zero_grad()
                    loss.backward()
                    torch.nn.utils.clip_grad_norm_(self.model.parameters(),cfg.TRAIN_CLIP_GRAD_NORM)
                    optimizer.step()
                    #scheduler.step()
                    
                    running_loss.update(loss.item(),bs)
                    running_loss1.update(loss_1.item(),bs)
                    running_loss2.update(loss_2.item(),bs)
                    if step%50==0:
                        #print(torch.cuda.memory_allocated())
                        #print(torch.cuda.max_memory_cached())
                        torch.cuda.empty_cache()
                        #torch.cuda.reset_max_memory_allocated()
                        print('step:{},now_lr:{} ,loss:{:.4f}({:.4f}),loss1:{:.4f}({:.4f}),loss2:{:.4f}({:.4f})'.format(step,now_lr ,running_loss.val,running_loss.avg,running_loss1.val,running_loss1.avg,running_loss2.val,running_loss2.avg))
                    #    print('step:{}'.format(step))
                        ...
                        if cfg.TRAIN_TBLOG:
                            ...
                            tblogger.add_image('Ref/Image', show_ref_img, step)
                            tblogger.add_image('Prev/Image', show_img1, step)
                            tblogger.add_image('Current/Image', show_img2, step)
                            tblogger.add_image('Current/GroudTruth', show_gtf, step)
                            tblogger.add_image('Current/Predictlabel', show_preds_sf, step)
...
##                    tmp_dic = self.model(inputs,ref_label,prev_label,seq_names=seqnames,gt_ids=obj_num,k_nearest_neighbors=cfg.KNNS)
                    ...
                    ################
                    t1 = time.time()
                    tmp,x1,x2,x3 = self.model.extract_feature(inputs)
                    _,_,x1 = torch.split(x1,split_size_or_sections=int(x1.size(0)/3), dim=0)
                    _,_,x2 = torch.split(x2,split_size_or_sections=int(x2.size(0)/3), dim=0)
                    _,_,x3 = torch.split(x3,split_size_or_sections=int(x3.size(0)/3), dim=0)
                    ...
                    edge,edge_fea = self.model.edge_layer(x1,x2,x3) 
                    if not cfg.MODEL_USE_EDGE:
                        input_edge=None
                    else:
                        input_edge=edge
                    t2 = time.time()
                    ...
                    current_frame_embedding,ref_label,prev_label,input_edge,
                    True,
                    seqnames,obj_num,cfg.KNNS,dynamic_seghead=self.model.dynamic_seghead)
                    t3 = time.time()
...
#                    cv2.imwrite(os.path.join('./result_2',seq_name,imgname[0].split('.')[0]+'.png'),pred_label)
                    ...
                    #exit()
                    #pred_edge = nn.functional.interpolate(pred_edge,size=(h,w),mode = 'bilinear',align_corners=True)
#                    pred_edge = torch.sigmoid(pred_edge)*255.
            
                    pred_edge = pred_edge.squeeze().numpy()
                    im2 = Image.fromarray(pred_edge.astype('uint8')).convert('L')
#                    im2.putpalette(_palette)
                    if not os.path.exists(os.path.join(cfg.RESULT_ROOT,'edge',seq_name)):
                        os.makedirs(os.path.join(cfg.RESULT_ROOT,'edge',seq_name))
                    im2.save(os.path.join(cfg.RESULT_ROOT,'edge',seq_name,imgname[0].split('.')[0]+'.png'))
