...
from networks.IntVOS_triplet import IntVOS
...
#        self.model.freeze_bn()
    ...
    def train(self,damage_initial_previous_frame_mask=True,lossfunc='cross_entropy',model_resume=False):
        ...
        running_triloss=AverageMeter()
        running_segloss=AverageMeter()
...
#                        shuffle=False,num_workers=cfg.NUM_WORKER,pin_memory=True)
        ...
        if model_resume:
            saved_model_=os.path.join(self.save_res_dir,'save_step_80000.pth')
 ...
 #       sample['meta']={'seq_name':seqname,'frame_num':frame_num,'obj_num':obj_num}
            for ii, sample in enumerate(trainloader):
                ...
                tmp_dic,tri_loss = self.model(inputs,ref_scribble_labels,label1s,label2s,seq_names=seq_names,gt_ids=obj_nums,k_nearest_neighbors=cfg.KNNS)
                ...
                seg_loss = criterion(tmp_dic,label_dic,step)
                seg_loss =seg_loss/bs
                loss = seg_loss+cfg.TRAIN_LAMBDA*tri_loss
                optimizer.zero_grad()
                loss.backward()
                ...
                if isinstance(tri_loss,float):
                    running_triloss.update(tri_loss,bs)
                else:
                    running_triloss.update(tri_loss.item(),bs)
                running_segloss.update(seg_loss.item(),bs)
                if step%50==0:
                    ...
                    print('seg_loss:{:.4f}({:.4f})'.format(running_segloss.val,running_segloss.avg))
                    print('tri_loss:{:.4f}({:.4f})'.format(running_triloss.val,running_triloss.avg))
                #    print('step:{}'.format(step))
                    ...
                    if cfg.TRAIN_TBLOG:
                        ...
                        tblogger.add_scalar('loss/loss', running_loss.avg, step)
                        tblogger.add_scalar('loss/seg_loss', running_segloss.avg, step)
                        tblogger.add_scalar('loss/tri_loss', running_triloss.avg, step)
